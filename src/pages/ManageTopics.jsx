...
export default function ManageTopics() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['topics']); resetForm(); }
  });
  ...
  const linkMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['topic-questions-all']); setShowLinkDialog(false); setSelectedQuestions(new Set()); }
  });
  const unlinkMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries(['topic-questions-all'])
  });
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 h-8 rounded flex items-center justify-center transition-all ${
                          isSelected ? 'bg-indigo-600 text-white' : 'bg-zinc-800 text-slate-400 hover:bg-zinc-700'
                        }`}
                      >
